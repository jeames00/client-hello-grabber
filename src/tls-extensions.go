package main

import (
	"fmt"
	. "github.com/bradleyfalzon/tlsx"
	"log"
)

var supported_extns = []string{
	"44",
	"27",
	"28",
}

type MyClientHello struct {
	ClientHello
	KeyShareEntries     []uint16
	PSKKeyExchangeModes []uint8
	SupportedVersions   []uint16
	RenegotiationInfo   []string
	CompressCertAlg     []uint16
	RecordSizeLimit     uint16
	ExtensionList       []int
	UnrecognisedExts    map[int][]uint8
	Headers             map[string][]string
}

const (
	ExtKeyShare            Extension = 51
	ExtPskKeyExchangeModes Extension = 45
	ExtSupportedVersions   Extension = 43
	ExtCompressCertificate Extension = 27
	ExtRecordSizeLimit     Extension = 28
)

var MyExtensionReg = map[Extension]string{
	ExtKeyShare:            "key_share",
	ExtPskKeyExchangeModes: "psk_key_exchange_modes",
	ExtSupportedVersions:   "supported_versions",
	ExtCompressCertificate: "compress_certificate",
	ExtRecordSizeLimit:     "record_size_limit",
}

func joinMaps() {
	for k, v := range ExtensionReg {
		MyExtensionReg[k] = v
	}
}

type JsonHello struct {
	Headers             map[string][]string `json:"headers"`
	HandshakeVersion    int                 `json:"handshake_version"`
	CipherSuites        []uint16            `json:"cipher_suites"`
	CompressMethods     []uint16            `json:"compression_methods"`
	ExtensionList       []int               `json:"extensions"`
	SupportedGroups     []uint16            `json:"supported_groups"`
	SupportedPoints     []uint16            `json:"supported_points"`
	ALPNs               []string            `json:"alpns"`
	OCSP                bool                `json:"ocsp"`
	SupportedVersions   []uint16            `json:"supported_versions"`
	SignatureAlgs       []uint16            `json:"signature_algorithms"`
	PSKKeyExchangeModes []uint16            `json:"psk_key_exchange_modes"`
	KeyShareEntries     []uint16            `json:"key_share_entries"`
	CompressCertAlg     []uint16            `json:"compress_certificate"`
	RecordSizeLimit     uint16              `json:"record_size_limit"`
	UnrecognisedExts    string              `json:"unrecognised_extensions"`
}

func NewJsonHello(mch *MyClientHello) JsonHello {
	var cipherSuites = make([]uint16, len(mch.CipherSuites))
	var compressMethods = make([]uint16, len(mch.CompressMethods))
	var supportedPoints = make([]uint16, len(mch.SupportedPoints))
	var pskKeyExchangeModes = make([]uint16, len(mch.PSKKeyExchangeModes))

	for i, v := range mch.CipherSuites {
		cipherSuites[i] = uint16(v)
	}

	for i, v := range mch.CompressMethods {
		compressMethods[i] = uint16(v)
	}

	for i, v := range mch.SupportedPoints {
		supportedPoints[i] = uint16(v)
	}

	for i, v := range mch.PSKKeyExchangeModes {
		pskKeyExchangeModes[i] = uint16(v)
	}

	return JsonHello{
		mch.Headers,
		int(mch.HandshakeVersion),
		cipherSuites,
		compressMethods,
		mch.ExtensionList,
		mch.SupportedGroups,
		supportedPoints,
		mch.ALPNs,
		mch.OSCP,
		mch.SupportedVersions,
		mch.SignatureAlgs,
		pskKeyExchangeModes,
		mch.KeyShareEntries,
		mch.CompressCertAlg,
		mch.RecordSizeLimit,
		fmt.Sprintf("%v", mch.UnrecognisedExts),
	}
}

func (ch *MyClientHello) Unmarshall(payload []byte) error {
	ch.Raw = payload
	ch.Type = uint8(payload[0])
	if len(payload[1:]) < 4 {
		log.Println("** Error: payload length less than 4")
		log.Printf("%v", ch.Type)
		return ErrHandshakeWrongType
	}
	ch.Version = Version(payload[1])<<8 | Version(payload[2])
	ch.MessageLen = uint16(payload[3])<<8 | uint16(payload[4])

	if ch.Type != uint8(22) {
		return ErrHandshakeWrongType
	}

	hs := payload[5:]

	if len(hs) < 6 {
		return ErrHandshakeBadLength
	}

	ch.HandshakeType = uint8(hs[0])

	if ch.HandshakeType != 1 {
		return ErrHandshakeWrongType
	}
	ch.HandshakeLen = uint32(hs[1])<<16 | uint32(hs[2])<<8 | uint32(hs[3])
	ch.HandshakeVersion = Version(hs[4])<<8 | Version(hs[5])

	hs = hs[6:]

	if len(hs) < ClientHelloRandomLen {
		return ErrHandshakeBadLength
	}

	// Get random data
	ch.Random = hs[:ClientHelloRandomLen]

	hs = hs[ClientHelloRandomLen:]

	if len(hs) < 1 {
		return ErrHandshakeBadLength
	}

	// Get SessionID
	ch.SessionIDLen = uint32(hs[0])
	hs = hs[1:]

	if len(hs) < int(ch.SessionIDLen) {
		return ErrHandshakeBadLength
	}

	if ch.SessionIDLen != 0 {
		ch.SessionID = hs[:ch.SessionIDLen]
	}

	hs = hs[ch.SessionIDLen:]

	if len(hs) < 2 {
		return ErrHandshakeBadLength
	}

	// Cipher Suite
	ch.CipherSuiteLen = uint16(hs[0])<<8 | uint16(hs[1])

	numCiphers := ch.CipherSuiteLen / 2

	if len(hs) < int(ch.CipherSuiteLen) {
		return ErrHandshakeBadLength
	}

	ch.CipherSuites = make([]CipherSuite, numCiphers)
	for i := 0; i < int(numCiphers); i++ {
		ch.CipherSuites[i] = CipherSuite(hs[2+2*i])<<8 | CipherSuite(hs[3+2*i])
	}

	hs = hs[2+ch.CipherSuiteLen:]

	if len(hs) < 1 {
		return ErrHandshakeBadLength
	}

	// Compression Methods
	numCompressMethods := int(hs[0])

	if len(hs) < 1+numCompressMethods {
		return ErrHandshakeBadLength
	}

	ch.CompressMethods = make([]uint8, numCompressMethods)
	for i := 0; i < int(numCompressMethods); i++ {
		ch.CompressMethods[i] = uint8(hs[1+1*i])
	}

	hs = hs[1+numCompressMethods:]

	if len(hs) < 2 {
		// No extensions or malformed length
		return ErrHandshakeBadLength
	}

	// Extensions
	ch.ExtensionLen = uint16(hs[0])<<8 | uint16(hs[1])

	if len(hs) < int(ch.ExtensionLen) {
		return ErrHandshakeExtBadLength
	}

	hs = hs[2:]
	ch.Extensions = make(map[Extension]uint16)
	ch.UnrecognisedExts = make(map[int][]uint8)

	for len(hs) > 0 {
		if len(hs) < 4 {
			return ErrHandshakeExtBadLength
		}

		extType := Extension(hs[0])<<8 | Extension(hs[1])
		length := uint16(hs[2])<<8 | uint16(hs[3])

		if len(hs) < 4+int(length) {
			return ErrHandshakeExtBadLength
		}

		data := hs[4 : 4+length]
		hs = hs[4+length:]

		ch.Extensions[extType] = length
		ch.ExtensionList = append(ch.ExtensionList, int(extType))

		switch extType {
		// 0
		case ExtServerName:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}
			sniLen := int(data[0])<<8 | int(data[0])

			data = data[2:]

			if len(data) < sniLen {
				// Malformed SNI data
				return ErrHandshakeExtBadLength
			}

			for len(data) > 0 {
				nameType := data[0]

				if len(data) < 3 {
					// Malformed ServerName
					return ErrHandshakeExtBadLength
				}

				nameLen := int(data[1])<<8 | int(data[2])

				data = data[3:]

				switch nameType {
				case SNINameTypeDNS:
					ch.SNI = string(data)
				default:
					// Unknown Name Type
				}
				data = data[nameLen:]
			}
		// 13
		case ExtSignatureAlgs:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}
			sigLen := int(data[0])<<8 | int(data[1])

			data = data[2:]

			if len(data) < sigLen {
				return ErrHandshakeExtBadLength
			}

			ch.SignatureAlgs = make([]uint16, sigLen/2)

			for i := 0; i < sigLen/2; i++ {
				ch.SignatureAlgs[i] = uint16(data[i*2])<<8 | uint16(data[i*2+1])
			}
		// 10
		case ExtSupportedGroups:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}
			groupLen := int(data[0])<<8 | int(data[1])

			data = data[2:]

			if len(data) < groupLen {
				// Malformed length
				return ErrHandshakeExtBadLength
			}

			ch.SupportedGroups = make([]uint16, groupLen/2)
			for i := 0; i < groupLen/2; i++ {
				ch.SupportedGroups[i] = uint16(data[i*2])<<8 | uint16(data[i*2+1])
			}
		// 11
		case ExtECPointFormats:
			if len(data) < 1 {
				return ErrHandshakeExtBadLength
			}
			pointLen := int(data[0])

			data = data[1:]

			if len(data) < pointLen {
				return ErrHandshakeExtBadLength
			}

			ch.SupportedPoints = make([]uint8, pointLen)
			for i := 0; i < pointLen; i++ {
				ch.SupportedPoints[i] = uint8(data[i])
			}

		// 5
		case ExtStatusRequest:
			if len(data) < 1 {
				return ErrHandshakeExtBadLength
			}

			switch data[0] {
			case OCSPStatusRequest:
				ch.OSCP = true
			}
		// 16
		case ExtALPN:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}

			alpnLen := int(data[0])<<8 | int(data[1])
			data = data[2:]

			if len(data) != alpnLen {
				return ErrHandshakeExtBadLength
			}

			for len(data) > 0 {
				stringLen := int(data[0])
				data = data[1:]
				ch.ALPNs = append(ch.ALPNs, string(data[:stringLen]))
				data = data[stringLen:]
			}

		// 51
		case ExtKeyShare:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}

			data = data[2:]

			for len(data) > 0 {
				group := uint16(data[0])<<8 | uint16(data[1])
				keyLen := int(data[2])<<8 | int(data[3])
				ch.KeyShareEntries = append(ch.KeyShareEntries, group)
				data = data[keyLen+4:]
			}

		// 45
		case ExtPskKeyExchangeModes:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}

			numModes := int(data[0])
			ch.PSKKeyExchangeModes = make([]uint8, numModes)
			data = data[1:]

			for i := 0; i < numModes; i++ {
				mode := uint8(data[0])
				ch.PSKKeyExchangeModes[i] = mode
				data = data[1:]
			}

		// 43
		case ExtSupportedVersions:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}

			numVersions := int(data[0]) / 2
			ch.SupportedVersions = make([]uint16, numVersions)

			data = data[1:]

			for i := 0; i < numVersions; i++ {
				version := uint16(data[0])<<8 | uint16(data[1])
				ch.SupportedVersions[i] = version
				data = data[2:]
			}
		// 65281
		case ExtRenegotiationInfo:
			if len(data) < 1 {
				return ErrHandshakeExtBadLength
			}

			renegInfoLen := int(data[0])

			if renegInfoLen > 0 {
				ch.RenegotiationInfo = append(ch.RenegotiationInfo, string(data[:renegInfoLen]))
			}

		// 27
		case ExtCompressCertificate:
			if len(data) < 3 {
				return ErrHandshakeExtBadLength
			}

			algLength := int(data[0]) / 2
			data = data[1:]

			ch.CompressCertAlg = make([]uint16, algLength)
			for i := 0; i < algLength; i++ {
				ch.CompressCertAlg[i] = uint16(data[i*2])<<8 | uint16(data[i*2+1])
			}

		// 28
		case ExtRecordSizeLimit:
			if len(data) < 2 {
				return ErrHandshakeExtBadLength
			}

			ch.RecordSizeLimit = uint16(data[0])<<8 | uint16(data[1])

			//		ch.RecordSizeLimit = make([]uint16, length)
			//		for i := 0; i < int(length)/2; i++ {
			//			ch.RecordSizeLimit[i] = uint16(data[i*2])<<8 | uint16(data[i*2+1])
			//			}

		default:
			// Other extension where we only care about presence, or presence
			// and length or unknown extension
			ch.UnrecognisedExts[int(extType)] = []uint8(data[:length])
		}

	}

	return nil
}
